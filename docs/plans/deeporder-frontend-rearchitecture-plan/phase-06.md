...
상태: DONE
완료 일시: 2026-07-11
Git commit: Phase 05 위에 연속 적용
이동 파일:
  - shared/components/floating/ActionMenu.tsx → components/layout/ActionMenu.tsx
  - shared/components/floating/PopoverPanel.tsx → components/layout/PopoverPanel.tsx
  - features/kds/store-status/components/StoreStatusControl.tsx → features/store-status/components/StoreStatusControl.tsx (import 경로 @/ alias 전환)
  - features/kds/store-status/components/StoreStatusDot.tsx → features/store-status/components/StoreStatusDot.tsx
  - features/kds/store-status/hooks/useStoreContext.ts → features/store-status/hooks/useStoreContext.ts (import @/ alias 전환)
  - shared/hooks/useKdsClock.ts → lib/date/use-clock.ts
생성 파일:
  - lib/notify.ts (sonner 기반 notify adapter, ShowToast re-export)
삭제 파일:
  - shared/components/KdsToast.tsx
  - shared/hooks/useToast.ts
  - features/kds/store-status/store-status.css
  - features/kds/layout/kds-actions.css
  - shared/components/floating/ (빈 디렉터리 제거)
toast 전환 방식:
  - App.tsx에 <Toaster position="bottom-center" richColors /> 추가 (sonner)
  - KdsPage.tsx에서 useToast/KdsToast 제거, notify를 showToast prop에 직접 전달
  - ShowToast 타입을 사용하는 5개 hook/component의 import를 lib/notify로 교체
부분 검증: typecheck PASS / lint PASS
blocker: 없음
```
